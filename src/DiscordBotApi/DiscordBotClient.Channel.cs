...
public partial class DiscordBotClient
{
	...
	)
	{
		...
		var argsDto = DiscordBulkDeleteMessagesArgsDto.FromModel(model: args);
		await _restClient
			.SendRequestAsync(
				requestFactoryFunc: () =>
				{
	...
	)
	{
		...
		var argsDto = DiscordCreateForumThreadArgsDto.FromModel(model: args);
		...
		var responseJson = await _restClient
			.SendRequestAsync(
				requestFactoryFunc: () =>
	...
	)
	{
		...
		var argsDto = DiscordCreateMessageArgsDto.FromModel(model: args);
		var payloadJson = _restClient.CreateJsonContent(value: argsDto);
	...
	)
	{
		...
		var argsDto = DiscordCreateThreadWithoutMessageArgsDto.FromModel(model: args);
	...
	)
	{
		...
		var argsDto = DiscordEditMessageArgsDto.FromModel(model: args);
		var payloadJson = _restClient.CreateJsonContent(value: argsDto);
	...
	)
	{
		...
		var messages = messageDtos.Select(selector: m => m.ToModel()).ToArray();
		return messages;
	}
	...
	)
	{
		...
		var responseDto = await _restClient
			.SendRequestAsync<DiscordThreadResponseDto>(
				requestFactoryFunc: () => new HttpRequestMessage(method: HttpMethod.Get, requestUri: builder.ToString()),
				cancellationToken: cancellationToken
			)
			.ConfigureAwait(continueOnCapturedContext: false);
	...
	)
	{
		...
		var argsDto = DiscordModifyGuildChannelArgsDto.FromModel(model: args);
		var channelDto = await _restClient
			.SendRequestAsync<DiscordChannelDto>(
				requestFactoryFunc: () =>
				{
	...
	)
	{
		...
		var argsDto = DiscordModifyThreadArgsDto.FromModel(model: args);
		var modifiedChannelDto = await _restClient
			.SendRequestAsync<DiscordChannelDto>(
				requestFactoryFunc: () =>
				{
					var request = new HttpRequestMessage(method: HttpMethod.Patch, requestUri: url);
		...
		var modifiedChannel = modifiedChannelDto.ToModel();
	...
	)
	{
		...
		var argsDto = DiscordStartThreadWithMessageArgsDto.FromModel(model: args);
		var channelDto = await _restClient
			.SendRequestAsync<DiscordChannelDto>(
				requestFactoryFunc: () =>
				{
